...
class Tool:  # noqa: D101 – simple value object
    ...
    def to_gemini_dict(self) -> dict[str, Any]:
        ...
        Gemini uses function declarations inside a ``Tool`` object.  The shape
        mirrors OpenAI's function object (``name``, ``description``,
        ``parameters``) but is passed via the ``tools`` config of the generate
        content request.
        """
        return {
            "name": self.name,
