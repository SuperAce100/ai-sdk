...
<CodeGroup>
  ```python Generate a greeting from ai_sdk import generate_text, openai model =
  openai("gpt-4o-mini") res = generate_text(model=model, prompt="Say hi to the world in one
  sentence") print(res.text) # "Hello, world – great to see you!" print(res.usage) #
  TokenUsage(prompt_tokens=4, completion_tokens=9, ...) ```
</CodeGroup>
